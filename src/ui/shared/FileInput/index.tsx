...
type Props = any & {
  name: { passportFileTypeId: number; token: "string" };
  label: string;
};

const FileInput = ({ name, label, passportFileTypeId, ...props }: Props) => {
  ...
  const mainValue = hasMethods ? methods.getValues(name) : "";
  ...
  useEffect(() => {
    if (mainValue?.name) {
      ...
      const fileExists = uploadedFiles.some((file) => file.name === mainValue.name);
  
      if (!fileExists) {
        setUploadedFiles((prevFiles) => [...prevFiles, testFile]);
      }
    }
  }, [mainValue]);
  ...
  const handleDeleteFile = () => {
    fileDelete.mutate(fileUpload.data?.data.fileToken);
  };
          ...
          uploadedFiles.map((file) => (
            <li key={file.size} className="flex gap-x-4 w-full px-2">
              ...
              <div className="flex gap-x-2 items-center">
                ...
                <Button
                  className="size-8 px-0 bg-red-100 hover:brightness-90 h-8"
                  onClick={(e) => (handleDeleteFile(), e.preventDefault())}
                >
