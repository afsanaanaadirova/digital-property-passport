...
export const SelectVM = <T extends SelectDataType>({
  ...
  const [rectLocation, setRectLocation] = useState<undefined | number>();
  const handleScroll = () => {
    if (selectRef.current) {
      const rect = selectRef.current.getBoundingClientRect().top;
      // setRectLocation(rect)
      setRectLocation(window.screen.availHeight - rect);
    }
  }
  ...
  return { innerValue, handleSelect, resetHandler, hasMethods, methods,selectRef, rectLocation, handleScroll, listsRef };
};
